...
setMethod("export.gff", "ANY", function(object, con, ...) {
    export(object, con, ...)
})
...
#' @exportMethod export
setMethod("export", c("ANY", "GFFFile"), function(object, con, format, ...) {
    if (hasMethod("asGFF", class(object))) object <- asGFF(object)
    ...
    object <- res
    if (!missing(format)) checkArgFormat(con, format)
    export(object, con, ...)
})
...
setMethod(
    "export",
    c("CompressedGRangesList", "GFFFile"),
...
setMethod(
    ...
    function(
        ...
        if (index) object <- .sortBySeqnameAndStart(object)

        seqname <- seqnames(object)
        if (is.null(mcols(object)$ID)) mcols(object)$ID <- names(object)
        if (version == "3")
            seqname <- urlEncode(seqname, "a-zA-Z0-9.:^*$@!+_?|-")
        if (!is.null(mcols(object)$source) && missing(source))
            source <- mcols(object)$source else
            source <- rep(source, length(object))
        if (version == "3") source <- urlEncode(source, "\t\n\r;=%&,", FALSE)
        feature <- mcols(object)$type
        if (is.null(feature)) feature <- rep("sequence_feature", length(object))
        score <- score(object)
        if (is.null(score)) {
        ...
        if (is.null(frame)) {
            ...
            if ("CDS" %in% feature)
                warning(wmsg(
                    "The phase information is missing. ",
            ...
            if (anyNA(frame[feature %in% "CDS"]))
                warning(wmsg(
                    "The phase information is missing for some CDS. ",
        ...
        if (version == "1") {
            ...
            if (is.null(attrs)) attrs <- as.vector(seqname)
        } else {
            ...
            if (length(custom)) {
                ...
                lattrs <- lapply(custom, function(name) {
                    x <- attrs[[name]]
                    x_flat <- if (is(x, "List"))
                        unlist(x, use.names = FALSE) else x
                    x_char <- as.character(x_flat)
                    x_char <- sub(
                        " *$",
                        "",
                        sub("^ *", "", as.character(x_char))
                    )
                    if (version == "3")
        ...
        if (!is.null(attrs)) {
            # write out the rows with attributes first
            write.table(
                cbind(table, attrs)[!is.na(attrs), ],
                con,
                sep = "\t",
                na = ".",
                quote = FALSE,
...
setMethod("import.gff", "ANY", function(con, ...) {
    import(con, "gff", ...)
})
...
setMethod(
    ...
    function(
        ...
        if (!missing(format)) checkArgFormat(con, format)
        if (!missing(version)) con <- asGFFVersion(con, match.arg(version))
        ...
        version <- gffFileVersion(con)
        if (!length(version)) {
            if (is.null(sniffed)) sniffed <- "1"
            con <- asGFFVersion(con, sniffed)
        }

        if (
            length(version) && !is.null(sniffed) && !identical(sniffed, version)
        )
            warning(
        ...
        ans <- readGFFAsGRanges(
            resource,
            version = version,
            colnames = colnames,
            filter = list(type = feature.type),
            genome = genome,
            sequenceRegionsAsSeqinfo = sequenceRegionsAsSeqinfo,
...
#'
#' @export
setGeneric("import.gff1", function(con, ...) standardGeneric("import.gff1"))

#' @describeIn GFFFile-class
#'
#' @exportMethod import.gff1
setMethod("import.gff1", "ANY", function(con, ...) import(con, "gff1", ...))

#' @describeIn GFFFile-class
#'
#' @export
setGeneric("import.gff2", function(con, ...) standardGeneric("import.gff2"))

#' @describeIn GFFFile-class
#'
#' @exportMethod import.gff2
setMethod("import.gff2", "ANY", function(con, ...) import(con, "gff2", ...))

#' @describeIn GFFFile-class
#'
#' @export
setGeneric("import.gff3", function(con, ...) standardGeneric("import.gff3"))

#' @describeIn GFFFile-class
#'
#' @exportMethod import.gff3
setMethod("import.gff3", "ANY", function(con, ...) import(con, "gff3", ...))
